...
## 8. 중간 업데이트 (2026-07-15) — 축 방향 재조정

Phase 1(DESeq2, padj<0.01 & |log2FC|>1.5) 결과: Liver 86개, LV(심장) 20개 유의 DEG.
Phase 2 1단계(CellChatDB 매칭) + 2단계(STRING PPI, confidence≥0.7) 결과, **당초 가설이었던
간→심장 방향은 두 방법 모두에서 매칭 0건**이었다. 대신 **심장→간 방향**에서 CellChatDB가
명확한 축을 하나 제시했다: **Angptl4(LV에서 유의 상향) → Cdh5/Sdc1/Sdc2/Sdc3/Sdc4(간에서 발현)**.

CellChatDB는 조직 내 근거리 신호전달(Wnt/Notch/TGFb/chemokine) 위주로 큐레이션되어 있어
고전적 내분비성 hepatokine 생물학을 놓치기 쉽고, STRING(더 넓은 근거 채널)에서도 간→심장
방향 교차 엣지가 없었다는 점은 이 데이터셋/통계 기준에서 그 방향의 직접 분자 연결이
데이터베이스 차원에서 뒷받침되지 않는다는 정직한 결과로 해석한다.

**방향 재조정 (사용자 확인, 2026-07-15):** Angptl4→Sdc/Cdh5 축(심장→간 방향)을 메인으로
채택한다. 이는 임상적으로 이미 확립된 **congestive hepatopathy / cardiohepatic syndrome**
(심부전이 거꾸로 간 손상을 유발하는) 개념과 정확히 부합하며, 애초 기획했던 "MASLD가
HFpEF를 유발"이라는 방향의 반대편 축을 다루게 된다. 간→심장 방향의 null 결과는 논문
